...
const getIdCajaMayor = async (req, res) => {
    try {
        const {id} = req.params;
        const listarCajaMayor = await pool.query('SELECT caj.CJA_ID, caj.CJA_ORIGEN_CUENTA,cue.CTA_NOMBRE AS CTA_NOMBRE1, caj.CJA_DESTINO, caj.CJA_CONCEPTO, caj.CJA_INGRESO, caj.CJA_EGRESO, caj.CJA_SALDO, caj.CJA_FECHA, caj.CJA_OBSERVACION  FROM cajamayor AS caj INNER JOIN cuentas AS cue ON caj.CJA_ORIGEN_CUENTA = cue.CTA_ID  WHERE caj.CJA_ID = ?',[id]);
