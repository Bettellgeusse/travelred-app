...
const getReservas = async (req, res) => {
    ...
    const verReserva = await pool.query('SELECT cliente.CLN_NOMBRE, cliente.CLN_CEDULA, cliente.CLN_CELULAR, tour.TOU_NOMBRE, tour.TOU_FECHA, cuentas.CTA_NOMBRE,  RES_ACOM, RES_EXTRA, RES_ABONO1,RES_ABONO2,RES_ABONO3,RES_OBSERVACIONES,RES_FECHA_ABONO1,RES_FECHA_ABONO2,RES_FECHA_ABONO3 From cliente INNER JOIN reserva ON reserva.CLN_ID = cliente.CLN_ID INNER JOIN tour ON reserva.TOU_ID = tour.TOU_ID INNER JOIN cuentas ON reserva.CTA_ID = cuentas.CTA_ID;');
