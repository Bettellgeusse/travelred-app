...
const getIdReservas = async (req, res) => {
    try {
        const {id} = req.params;
        const listarReserva = await pool.query('SELECT r.RES_ID, r.GRP_ID, (SELECT CLN_NOMBRE FROM cliente WHERE CLN_ID = g.CLN_ID ) AS LIDER,(SELECT ROL_NOMBRE FROM rol WHERE ROL_ID = r.ROL_ID ) AS USUARIO_ROL, g.CLN_ID AS LIDER_G, g.GRP_VALORTOTAL, g.GRP_VALORSALDO,r.RES_VALOR_UNITARIO,r.RES_VALOR_TOTAL,  r.RES_ACOM,r.RES_ACOM_CUENT,cueacom.CTA_NOMBRE AS CTA_NOMBRE_ACOM,r.RES_EXTRA,r.RES_EXTRA_CUENT,cuextra.CTA_NOMBRE AS CTA_NOMBRE_EXTRA,r.RES_ABONO1,r.RES_ABONO2,r.RES_ABONO3,r.RES_OBSERVACIONES,r.RES_OBSERVACIONES_FACTURA, r.RES_FECHA_ABONO1,r.RES_FECHA_ABONO2,r.RES_FECHA_ABONO3, r.RES_FECHA_ABONO4,r.RES_FECHA_ABONO5, r.RES_CUEN_ABONO1,cue.CTA_NOMBRE AS CTA_NOMBRE1,r.RES_CUEN_ABONO2,cue2.CTA_NOMBRE AS CTA_NOMBRE2,r.RES_CUEN_ABONO3,cue3.CTA_NOMBRE AS CTA_NOMBRE3,r.RES_CUEN_ABONO4,cue4.CTA_NOMBRE AS CTA_NOMBRE4,r.RES_CUEN_ABONO5,cue5.CTA_NOMBRE AS CTA_NOMBRE5,c.CLN_ID, c.CLN_NOMBRE,c.CLN_CEDULA,c.CLN_CELULAR,t.TOU_ID, t.TOU_NOMBRE,t.TOU_FECHA,t.TOU_VALORNETO, t.TOU_VALORCOMISIONABLE, t.TOU_INFANTE, t.TOU_NINO, t.TOU_ADULTO, t.TOU_ADULTO_MAYOR, t.TOU_AGENCIAS, t.TOU_CODIGO_AEROLINEA,t.TOU_VUELO_SALIDA,t.TOU_HORA_SALIDA,t.TOU_VUELO_REGRESO,t.TOU_HORA_REGRESO FROM reserva AS r INNER JOIN cliente AS c ON r.CLN_ID = c.CLN_ID INNER JOIN tour AS t ON r.TOU_ID = t.TOU_ID INNER JOIN cuentas AS cue ON r.RES_CUEN_ABONO1 = cue.CTA_ID INNER JOIN cuentas AS cue2 ON r.RES_CUEN_ABONO2 = cue2.CTA_ID INNER JOIN cuentas AS cue3 ON r.RES_CUEN_ABONO3 = cue3.CTA_ID INNER JOIN cuentas AS cue4 ON r.RES_CUEN_ABONO4 = cue4.CTA_ID INNER JOIN cuentas AS cue5 ON r.RES_CUEN_ABONO5 = cue5.CTA_ID INNER JOIN cuentas AS cueacom ON r.RES_ACOM_CUENT = cueacom.CTA_ID INNER JOIN cuentas AS cuextra ON r.RES_EXTRA_CUENT = cuextra.CTA_ID INNER JOIN grupo AS g ON r.GRP_ID = g.GRP_ID = g.GRP_ID WHERE RES_ID = ?',[id]);
